...
function App() {
  ...
  const [expiringFood, setExpiringFood] = useState<ExpiringFood[]>([]);
  ...
  setExpiringFood(prev => {
    const updated = prev.filter(f => f.name !== item); // ⚠️ utiliser "name" ici
    return [...updated, { name: item, expirationDate: date }]; // ⚠️ clé "name"
  });
};
